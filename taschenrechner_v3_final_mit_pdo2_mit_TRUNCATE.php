...
function printDatabaseBillig($pdo)
{
	...
	echo '<form method="POST" action="taschenrechner_v3_final_mit_pdo2_mit_TRUNCATE.php">';
...
function printDatabaseBerechnungen($pdo)
{
	...
	foreach ($pdo->query($pdoSQL3) as $row2)
	{
		...
		echo "<td>" . '<button name="btn" value= "'.$row2["ID"].'" >Löschen</button>' . "</td>";
		echo "</tr>";
	}
	echo '<button name ="dropTable">Ganze Tabelle löchen</button>';
	echo '</table>';
	echo '</form>';
}
...
function checkSet($pdo, $result)
{
	...
	if(isset($_POST["dropTable"]))
	{
		...
		echo '<br>Tabelleninhalt könnte nicht gelöscht werden';
	}
}
...
<form action="taschenrechner_v3_final_mit_pdo2_mit_TRUNCATE.php" method="POST">
